...
for folder in $(ls -d */ | cut -f1 -d'/'); do
	...
	for article in $(ls $folder/*.md); do
		...
		if echo $article | grep -q index.md; then
			continue
		fi
		url="./$article"
		...
		image="./${folder}/$(cat $article | grep -e media -e images | grep -e ".png" -e".svg" -e ".jpg" | head -1 | sed -n 's/.*\(\(media\|_images\|images\).*\(svg\|png\|jpg\)\).*/\1/p')"
